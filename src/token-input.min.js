(function(b,a){if(typeof define==="function"&&define.amd){define(["vendor/zepto.min"],a);}else{b.TokenInput=a(b.Zepto);}})(this,function(c){var a={values:[],use_data_only:true,use_duplicate:false,max_result:4},d=function(){},b=function(l){var p=function(){var r=this,C=c(r.elems[0]),y=c('<ul class="token-input"></ul>'),i=c('<li class="input input-ing"></li>'),F=c('<input type="text" autocomplete="off" />'),I=c('<div class="autocomplete"></div>'),B=c('<li class="input input-token"></li>'),A=c('<input type="hidden" name="'+r.name+'" />'),K=false,v=0,D=0,j=function(){C.before(y);
...
switch(S){case 13:case 108:case 188:Q.preventDefault();if(T===""){return;}if(l.use_data_only){var U=I.find("div.hover");if(U.length){L(U.text(),U.data("value"));
G();}}else{L(F.val(),F.val());G();}break;case 40:case 38:Q.preventDefault();if(I.hasClass("active")){var R=I.find("div.hover"),O;if(S==40){O=R.next();}else{O=R.prev();
}if(O.length){R.removeClass("hover");O.addClass("hover");}}break;case 37:case 39:Q.preventDefault();var P=t();if(P.length!==0){var N;if(S===37){N=P.prev();
}else{N=P.next();}if(N.length!==0){N.addClass("hover").siblings().removeClass("hover");if(N.hasClass("input-token")){N.addClass("hover");}N.siblings().removeClass("hover");
}}else{if(S===37){y.find("li.input-token").eq(-1).addClass("hover");}}break;case 27:Q.preventDefault();E();break;case 8:if(T===""){Q.preventDefault();var P=t();
if(P.length===0){var M=y.find("li.input-token").eq(-1);if(!M.hasClass("hover")){M.addClass("hover");}}else{u();}break;}default:if(!l.use_data_only){break;
}if(v&&!K){clearTimeout(v);}K=false;v=setTimeout(function(){K=true;x.apply(V,arguments);},0);break;}},x=function(R){var Q=c(this).val(),P=new RegExp(Q,"i"),M=[],S=[];
